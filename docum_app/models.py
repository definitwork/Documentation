...
class SubCategory(models.Model):
    doc_category = models.ForeignKey(DocCategory, on_delete=models.CASCADE, verbose_name='Категория')
    subcategory_title = models.CharField(max_length=255, verbose_name='Заголовок подкатегории', unique=True)
    description = models.TextField(verbose_name='Описание подкатегории', blank=True, null=True)
    slug = models.SlugField(unique=True, verbose_name='URL')
    video = models.FileField(upload_to='video/sub_category', verbose_name='Видео', blank=True, null=True)
    cover_img = models.ImageField(upload_to='img/sub_category', verbose_name='Картинка, покрывающая видео', blank=True,
                                  null=True)
...
class CategoryContent(models.Model):
    ...
    sub_categories = models.ManyToManyField('SubCategory',
                                            verbose_name='Связанные подкатегории')
    ...
    class Meta:
        verbose_name_plural = 'Статьи подкатегорий документации'
        verbose_name = 'Статья подкатегорий документации'
